...
const wsProvider = new WebsocketProvider('ws://localhost:1234', 'nested', yDoc);
const yColumns = yDoc.getArray('columnMap');
...
function NestedMap() {
  ...
  function rebuildSpreadsheet() {
    const newHeaders = [];
    yColumns.forEach(map => newHeaders.push(map.get('header')))
    ...
    yColumns.forEach((map,colIndex) => {
      for (const [key,value] of map) {
        if (key.length > 6) {
          const rowIndex = yRows.toArray().indexOf(key);
  ...
  const handleHeaderBlur = (colIndex, value) => {
    yColumns.get(colIndex).set('header',value)
  }
  ...
  const handleCellBlur = (rowIndex, colIndex, value) => {
    if ((value === '') && (!(yColumns.get(colIndex).has(yRows.get(rowIndex))))) return;
    yColumns.get(colIndex).set(yRows.get(rowIndex),value);
  }
  ...
  function handleColumnContextMenuClick (index) {
    switch(index) {
      ...
      case 2:
        ...
        break;
      case 1:
        yColumns.delete(columnOpenIndex);
        break;
      default:
  ...
  return (
    <div className="split">
      ...
      <tt className='json'>
        <div>
          <b>Column Array maps to header and cell data (yArray of yMaps): </b><br />
