...
const wsProvider = new WebsocketProvider('ws://localhost:1234', 'removeWins', yDoc);
...
function FixedElements() {
  ...
  useEffect(() => {
    ...
    yMap.observe((event) => {
      // console.log('Data',event,event.transaction.origin);
      rebuildSpreadsheet();
    });
    // yArray observers. Handle structural changes
    yColumns.observe((event) => {
      // console.log('Col',event,event.transaction.origin);
      rebuildSpreadsheet();
    });
    yRows.observe((event) => { 
      // console.log('Row',event,event.transaction.origin);
      rebuildSpreadsheet();
    });
  }, []);
  ...
  const handleCellBlur = (rowIndex, cellIndex, value) => {
    if (value === '') {
      if (!(yMap.get(yRows.get(rowIndex) + ',' + yColumns.get(cellIndex)))) return;
    }
